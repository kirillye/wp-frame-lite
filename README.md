...
`inc/security.php` закрывает базовое, не претендуя на замену плагину безопасности:

- **Перечисление авторов** закрыто со всех трёх сторон: `?author=1` отдаёт 403, архивы авторов уходят на главную, `/wp-json/wp/v2/users` скрыт от неавторизованных (авторизованным с правом `list_users` он нужен — его использует редактор блоков), из ответа oEmbed вычищены `author_name` и `author_url`.
- **Заголовки ответа**: `X-Content-Type-Options`, `X-Frame-Options`, `Referrer-Policy`, `Permissions-Policy`. Набор фильтруется:

```php
add_filter( 'wp_frame_lite_security_headers', function ( array $headers ): array {
	$headers['X-Frame-Options'] = 'DENY';
	return $headers;
} );
```

CSP и HSTS намеренно не ставятся: первая слишком зависит от набора скриптов конкретного проекта, вторая должна настраиваться на сервере — при неполном HTTPS она способна сделать сайт недоступным.

- **SVG-загрузки** разрешены только тем, кто и так может загружать файлы, и проходят через санитайзер: вырезаются `script`, `use`, анимационные теги, атрибуты `on*` и ссылки `javascript:` в `href`/`xlink:href`.

Все обработчики — именованные функции. Тему форкают под клиента, и лишнее должно сниматься штатно:

```php
remove_action( 'template_redirect', 'wp_frame_lite_redirect_author_archive' );
```

## Доступность

Помимо скип-линка и видимого фокуса тема уважает системную настройку «уменьшить движение» — её включают при вестибулярных расстройствах, где анимации вызывают тошноту.

`utilities/motion.css` сжимает переходы и анимации до 0.01ms вместо полного отключения: событие `transitionend` продолжает срабатывать и зависящий от него JS не ломается. Swiper двигает слайды JS-трансформациями, поэтому CSS его не остановит — модуль слайдера отдельно выставляет `speed: 0` и выключает автопрокрутку.
...
Забыть про это легко, поэтому за правилом следит CI. `.github/workflows/ci.yml` пересобирает проект и падает, если `assets/dist/css/main.css` или `assets/dist/js/main.js` разошлись с закоммиченными.

Сверяются только эти два файла. Их преджатые двойники `.br`/`.gz` не проверяются: brotli зависит от версии zlib в Node и на другой версии даст другие байты без реальной причины. Шаг `images` в CI тоже пропущен — он инкрементален по mtime, а в свежем `checkout` у всех файлов одинаковое время.

Если на проекте появится полноценный CI с деплоем, разумнее наоборот: вернуть `assets/dist/` в `.gitignore` и собирать на выкладке.
